...
class ActionLayer(BaseActionLayer):
    ...
    def _inconsistent_effects(self, actionA, actionB):
        ...
        for effectA in self.children[actionA]:
            for effectB in self.children[actionB]:
                if str(effectA) == '~' + str(effectB) or str(effectB) == '~' + str(effectA):
                    return True
        return False
    ...
    def _interference(self, actionA, actionB):
        ...
        for effectA in self.children[actionA]:
            for precondB in self.parents[actionB]:
                if str(effectA) == '~' + str(precondB) or str(precondB) == '~' + str(effectA):
                    return True
        for effectB in self.children[actionB]:
            for precondA in self.parents[actionA]:
                if str(effectB) == '~' + str(precondA) or str(precondA) == '~' + str(effectB):
                    return True
        return False
    ...
    def _competing_needs(self, actionA, actionB):
        ...
        """
        for condA in self.parents[actionA]:
            for condB in self.parents[actionB]:
                if self.parent_layer.is_mutex(condA, condB):
                    return True
        return False
        ...
        for actionA in self.parents[literalA]:
            for actionB in self.parents[literalB]:
                if not self.parent_layer.is_mutex(actionA, actionB):
                    return False
        return all_literals_mutex

    def _negation(self, literalA, literalB):
        """ Return True if two literals are negations of each other """
        return str(literalA) == '~' + str(literalB) or str(literalB) == '~' + str(literalA)
...
class PlanningGraph:
        ...
        for g in self.goal:
            ...
            for i, layer in enumerate(self.literal_layers):
                if g in layer:
                    ...
                    break
            if not goal_exist:
                while g not in self.literal_layers[-1]:
                    self._extend()
                cost += self.literal_layers.__len__() - 1
        return cost
    ...
    def h_setlevel(self):
        ...
        while not self._is_leveled:
            #Check if the layer contains all goals literals #
            all_goals_met = True
            for g in self.goal:
                if g not in self.literal_layers[-1]:
                    all_goals_met = False
                    break

            # Check no pair of goal literals are mutex
            if all_goals_met:
                all_goals_mutex = True
                for g1, g2 in combinations(self.goal, 2):
                    if self.literal_layers[-1].is_mutex(g1, g2):
                        all_goals_mutex = False
                        break
                if all_goals_mutex:
                    return self.literal_layers.__len__() - 1
            self._extend()
        return -1
